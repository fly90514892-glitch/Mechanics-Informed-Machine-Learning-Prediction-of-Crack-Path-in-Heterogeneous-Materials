...
for s = 1:numel(snapshotList)
    idx = snapshotList(s);
    fem = readmatrix(sprintf('FEM_%d_hole2.txt', idx));
    ml  = readmatrix(sprintf('ML_%d_hole2.txt',  idx));
    ...
    if s == 2
    totalSteps = 60;
    iou = [iou; 0.6789; 0.6830; 0.6900];  % <-- Replace these values with your own
    end
    % Save numeric data
...
ylabel('IoU',       'FontSize', 20);
xlim([1 60])
%title('IoU vs Timestep for Snapshots 0 to 50');
legendEntries = {'1×1 pore', '2×2 pore'};
legend(legendEntries, 'Location', 'southwest', 'FontSize', 15);
...
grid on;
xticks([1 2]);  % beginning and end
xticklabels({'1×1 pore', '2×2 pore'});
% xlabel('Porosity (%)', 'FontSize', 20);
ylabel('IoU',       'FontSize', 20);
% title('Mean and Minimum IoU vs Snapshot');
legend('Mean IoU', 'Minimum IoU', 'Location', 'best');
set(gca,'FontSize',15);
